"""Rung-1 pretrain (spec 6, 16.5).

1B-scale active / ~15B MoE, 200B token target. Decides μP transfer, Muon
settings, hybrid ratio, MoE granularity. Same loop as rung 0: real ckpt
files, resume only when a later job loads a ckpt from a different Slurm id.
"""
...
TOKENS_TARGET = 200_000_000_000


def rung1_gpu_config() -> ModelConfig:
    """Hybrid 3:1, 24 layers. Roughly 1B active / 15B total."""
    return ModelConfig(
        d_model=1536,
        n_layers=24,
        n_dense=1,
...
def _config() -> ModelConfig:
    if os.environ.get("RUNG1_TINY"):
        return M.cpu_config()
    return rung1_gpu_config()


def mup_lr(base_lr: float, d_model: int, base_d: int = 768) -> float:
    """μP: LR transfers as base_lr * (base_d / d_model) for the hidden width."""
    return float(base_lr) * float(base_d) / float(max(d_model, 1))
...
def run(
    ...
    job_id = str(os.environ.get("SLURM_JOB_ID") or os.environ.get("RUNG1_JOB_ID") or "local")
    cfg = _config()
    tiny = bool(os.environ.get("RUNG1_TINY"))
    devices = jax.devices("gpu") if jax.default_backend() == "gpu" else jax.devices()
    n_devices = max(len(devices), 1)
    default_batch = "2" if tiny else "64"
    batch = int(batch if batch is not None else os.environ.get("RUNG1_BATCH", default_batch))
    default_seq = 8 if tiny else int(cfg.max_context)
    seq = int(seq if seq is not None else os.environ.get("RUNG1_SEQ", default_seq))
    seq = min(seq, int(cfg.max_context))
    peak = mup_lr(1e-3, cfg.d_model, base_d=768)
    tcfg = TrainConfig(peak_lr=peak)
    ...
    env_steps = os.environ.get("RUNG1_MAX_STEPS")
    ...
    return {
        ...
        "rung": 1,
        "mup_lr": peak,
        "hybrid_ratio": cfg.n_linear_attn / max(cfg.n_mla, 1),
