//! In-memory sandbox engine shared by [`crate::InProcess`] and [`crate::Pool`].
//! No sockets, no wall clock, no live network.
...
use crate::{
    Artifact, Error, Image, ImageId, NowMs, PoolConfig, Result, SandboxId, SnapshotId, TamperFlag,
    Tool, ToolRequest, ToolResponse, View, GRADER_ROOT, SCHEMA_TOOL_REQUEST, SCHEMA_TOOL_RESPONSE,
    SCHEMA_VERSION,
};
...
pub(crate) struct Engine {
    ...
    snapshots: BTreeMap<String, Snap>,
    sandboxes: BTreeMap<String, Sb>,
...
impl Engine {
    ...
    pub(crate) fn tamper_flags(&self) -> &[TamperFlag] {
        &self.tamper
    }
    ...
    pub(crate) fn snapshot_from_image(
        &mut self,
        image: &ImageId,
        _now: NowMs,
    ) -> Result<SnapshotId> {
        ...
        self.snapshots.insert(sid.0.clone(), Snap { files, env });
        Ok(sid)
    }
    ...
    pub(crate) fn boot(&mut self, image: &Image, _now: NowMs) -> Result<SandboxId> {
        ...
        let mut files = image.agent_files.clone();
        files.extend(image.hidden_tests.clone());
    ...
    pub(crate) fn fork_group(
        &mut self,
        snapshot: &SnapshotId,
        n: usize,
        _now: NowMs,
    ) -> Result<crate::ForkResult> {
    ...
    pub(crate) fn snapshot(&mut self, sandbox: &SandboxId, _now: NowMs) -> Result<SnapshotId> {
        let sb = self
            .sandboxes
            .get(&sandbox.0)
    ...
    pub(crate) fn kill(&mut self, sandbox: &SandboxId, _now: NowMs) -> Result<()> {
        let sb = self
            .sandboxes
            .get_mut(&sandbox.0)
            .ok_or(Error::SandboxNotFound)?;
    ...
    pub(crate) fn agent_view(&self, sandbox: &SandboxId) -> Result<BTreeMap<String, Vec<u8>>> {
    ...
    pub(crate) fn grader_view(&self, sandbox: &SandboxId) -> Result<BTreeMap<String, Vec<u8>>> {
        let sb = self.live_sb(sandbox)?;
        Ok(sb
            .files
            .iter()
    ...
    pub(crate) fn list(
        ...
        match view {
            View::Agent => self.agent_view(sandbox),
            View::Grader => self.grader_view(sandbox),
        }
    }

    fn live_sb(&self, sandbox: &SandboxId) -> Result<&Sb> {
        let sb = self
            .sandboxes
            .get(&sandbox.0)
            .ok_or(Error::SandboxNotFound)?;
    ...
    fn live_sb_mut(&mut self, sandbox: &SandboxId) -> Result<&mut Sb> {
        let sb = self
            .sandboxes
            .get_mut(&sandbox.0)
            .ok_or(Error::SandboxNotFound)?;
...
fn is_grader_path(path: &str) -> bool {
    path == GRADER_ROOT || path.starts_with("/grader/")
}

fn hidden_tests_hash(hidden: &BTreeMap<String, Vec<u8>>) -> String {
