"""Independent slow PyTorch V1 forward (spec 16.2).

Plain PyTorch. Not JAX, not ``model.forward``, not
``prometheus.verify._numpy_forward``, not ``prometheus.verify.v1_parity``.
Production must never import this module.

Math is the flagship-shape discrete net: hybrid 3:1 linear vs MLA, Gated
DeltaNet linear attention, DeepSeek-V3 MLA + QK-norm, sigmoid-gated MoE,
pre-norm RMSNorm, core block iterated ``r`` times. Returns next-token
logits ``(batch, seq, vocab_size)`` float32.

NumPy is used only to convert incoming tokens/params into torch tensors.
"""
...
def linear_attention(
    ...
    """Causal Gated-DeltaNet-family recurrence (delta rule, β=1). Slow loop."""
...
def node_limited_topk(
    ...
    """Greedy top-k with at most ``max_racks`` racks per token. Python loop."""
...
def forward(
    ...
    *,
    ...
    """Next-token logits ``(batch, seq, vocab_size)`` float32. Slow PyTorch."""
